...
Crowding_Plot <- ggplot(df_pre_drop, aes(Crowding_Perc, y = 0)) +
  ...
  labs(y = '', x = '% Crowding')
combined_plot <- (Minority_plot / Multi_Unit_Plot / Crowding_Plot) + labs(tile = 'Boxplots for selected socioeconomic features')
combined_plot
```
...
<!-- 3. What [covariates](https://en.wikipedia.org/wiki/Dependent_and_independent_variables#Statistics_synonyms) help you achieve your modeling goals? What covariates are problematic, either due to *collinearity*, or because they will absorb some of a causal effect you want to measure? -->
TODO: Add justifications of EDA to each of the covariate descriptions. Maybe talk about overarching EDA process.
...
\textbf{Socioeconomic}: TODO <!--@KEVIN CAN YOU ADD SOME STUFF HERE TY-->

\textbf{Miscellaneous}: The covariates for senior percentages and ICU beds per 10,000 actually appear to be of little concern. Neither seem to be overly problematic due to collinearity. 
...
\textbf{Policy}: For policy variables, no logarithmic or polynomial transformation conducted improved linear relationships between our explanatory variabless and fatality rate.

\textbf{Socioeconomic}: TODO: Describe logarithmic transformation applied to the data. Reference the scatterplot figure!!
...
plot3 <- ggplot(df, aes(y = Fatality_Rate, x = Minority_Percentage)) +
 geom_point() + stat_smooth() + labs(x = '% Minority', y = 'Fatality Rate', title = 'Fatality Rate vs % Minority without Transformation')
plot4 <- ggplot(df, aes(y = Fatality_Rate, x = log(Minority_Percentage))) +
 geom_point() + stat_smooth() + labs(x = '% Minority', y = 'Fatality Rate', title = 'Fatality Rate vs % Minority with Transformation')
plot3 / plot4
```
...
TODO: Introduction to what tests we ran and why?
...
```
Results:
  In the limited model, we can see that the mask_ratio variable is statistically significant due to its low p value <0.01, the model also returns unemployment percent as statistically significant.  With respect to practical significance, for every percent of the time the mask policy was enacted, all else held constant, we expect to see a .004% increase in fatality rates.  Which is counter intuitive considering we were expecting the mask policy to contribute to lowering COVID-19 deaths. Another variable that was tested to be statistically significant is the unemployment percentage.  The coefficient of 0.195 implies that without any effects from other variables a one percent in unemployment effectively increases the fatality rate by 19.5% thus pointing to a large practical significance.  
  
In the second model, unemployment percent and chronic respiratory disease death percent are shown to be statistically significant with a p value less than 0.01.  The variables of percent diagnosed with diabetes and the intercept are shown to be statistically significant but less so with a p value less than 0.1. With regards to practical significance, we see an increase in the coefficient for unemployment percent in the second model resulting in a stronger practical significance the variable would have on the fatality rate, given all else equal. We do see mask ratio lose its statistical significance while still maintaining practical significance to a degree.  Continuing with practical significance, we see that chronic respiratory disease death percent has an abnormally large coefficient where we can conclude that for every percent change in the death rate for chronic respiratory disease we expect to see a 21.7% decrease in fatality rate.  Lastly, we look at percent of population in each state diagnosed with diabetes. We can infer from the results that for every percent increase in the population that is diagnosed with diabetes, we see a 9.6% increase of fatality rate in the model.

In the third model, we see unemployment percent and percent diagnosed with diabetes continue to be statistically significant with p values less than 0.01 and 0.05 respectively.  We are also met with stroke death percent as another variable that is observed to be highly statistically significant.  The affect of stroke death percent is shown to have the highest practical significance, with an affect of 44% decrease in fatality rate for every percent increase, when compared to any other health variable measured on the same scale.  The chronic respiratory disease death rate is also perceived to no longer have statistical significance as well as lose some of it's practical significance with the inclusion of the additional health variables.  Finally, it is important to note that the increased statistical significance seen in the variable representing the percent of population diagnosed with diabetes also came with an increase to the practical significance with a factor of two resulting in a 18.4% increase in fatality rate for every increase in percent of the state population diagnosed with diabetes.

Comparing the three models, it is clear that model 2 does the best job of explaining COVID-19 fatality rates. The coefficients in unemployment percent is similar to that of the first model and with the inclusion of other variables such as chronic respiratory disease death percent allows us to improve our model by 28%. Comparing the models on the VIF chart, we can see a dramatic increase in VIF values jumping from model two to model three indicating high multicollinearity among the variables that originally existed within model two. Several variables are propelled to values higher than 4 with diabetes percent crossing over the threshold to 5.35. This means that adding the additional variables into the third model did not dramatically improve our model and might have caused underlying factors to be represented in multiple variables. Looking at the standard errors for each coefficient, asthma deaths percent stands out as having an exceedingly high standard error of 302.008 with a coefficient of -31.038. Chronic respiratory disease death percent also shows high mullticollinearity in the third model with a reduction in practical significance but a dramatic increase in standard error.
...
df %>% 
  ...
  geom_point() + 
  stat_smooth() + ggtitle("Model Two: Residuals vs. Predictions")
```
Our comments
...
TODO: Intro transitioning from what we have and results to considerations for OVB
| Regression                          |
|------------------------------------------------|
| $Fatality Rate = \beta_0 + \beta_1 maskRatio + \beta_2 DiabetesDiagnosedPerc  +\beta_3 UnemploymentPerc +\beta_4 HomeRatio +\beta_5 ChronicRespiratoryDiseaseDeathPerc +\beta_6 log(MinorityPerc) + u$ |
| Omitted Variables                   |
| $extremity\_of\_weather$            |
| $food\_quality$                     |
| $registered\_Republican$            |
| $distance\_from\_tourist\_hotspots$ |
| $perception\_of\_COVID\_severity$   |


1. "Food Quality": Quality and availability of healthy food throughout a state.
...
"Food Quality" will likely have a positive correlation with minority percentages, and as minority has a negative causal impact on fatality rates, the direction of the OVB will be towards zero, and we have overstated the impact of minority percentage.
(include more on how LARGE the bias is)
...
2. "Extreme Temperatures": Mean range of temperature changes across the state over the course of our dataset's time frame.
We will argue that more extreme temperature ranges will be positively correlated with fatality rates. It is well known that extreme temperature changes will weaken a person's immune system and their body's ability to fight a virus. This would imply that states with more moderate weather will have a lower fatality rates with COVID-19. A similar omitted variable that relates could be the humidity of the air-- as we know, virus' spread when there are lower humidity rates. This bias could be relatively large given the lower percentage of fatality rates in the first place. 
"Extreme Temperatures" will likely have no correlation with state mask policies and no correlation with state stay at home policies. Temperatures should not have an impact on state policies. "Extreme Temperatures" will also likely have no correlation with diabetes diagnoses, no correlation with unemployment percentages, no correlation with chronic respiratory disease percentages, and no correlation with minority percentages. Because we see weather and temperatures as uncorrelated with each of our independent variables, we can say that the direction of the OVB is towards zero? because of our assumption that it is positively correlated with fatality rates.

3. "% registered Republican": A higher percentage of Republican voters in a state.
We believe that political distributions of Republicans will lead to different state and government messages / resources concerning COVID-19 from state leaders. Downplaying the severity of COVID-19 by keeping available resources scarce could lead to uninformed state members, which should increase the fatality rate within a state.
We believe that "% registered Republican" will have a negative correlation with fatality rates. The bias of this would likely be smaller, but the at-risk population that believes in COVID's severity through government infrastrucutre and messagin will likely take more drastic measures to protect themselves.
"% registered Republican" will like have a strong negative correlation with state mask policies and stay-at-home policies."% registered Republican" will likely have no correlation with diabetes percentages and chronic respiratory percentages. This variable will likely have a strong negative correlation with minority percentages and unemployment rates.
This leads us to conclude that the bias for this OVB is towards zero?, as the correlations with the independent variables are evenly distributed, and we have reason to believe that there is a negative correlation with fatality rates.

4. "Distance from Tourist Hotspots": Aggregated Distance from Tourist Hotspots / High Frequency travel zones across a state.
We believe that distances from tourist hotspots / urban zones will likely have a negative correlation with fatality rates. A further distance that cities are from unpredictable and unwanted influxes of tourists would likely correlate with lower fatality rates.
"Distance from Tourist Hotspots" will likely have no correlation with state mask policies and stay-at-home policies. This variable will also likely have no correlation with minority percentages and unemployment rates, as well as with diabetes and chronic respiratory percentages. This variable is more related to unwanted influxes of tourists during COVID-19, and we will conclude that the direction of the bias is towards zero because of the 

5. "Perception of COVID-19 Severity": A survey value of the average perception of how severe / fatal COVID-19 is.
I believe that a higher perception of how severe / fatal COVID-19 is would lead to lower fatality rates. People who are more concerned about the severity of COVID-19 will likely take many more personal precautions to watch for their own health and isolate if they have underlying health conditions or are immunocompromised.
"Perception of COVID-19 Severity" will likely have a strong positive correlation with state mask policies, no correlation with diabetes diagnoses, no correlation with unemployment percentages, a positive correlation with state stay at home policies, no correlation with chronic respiratory disease percentages, and no correlation with minority percentages.
This leads us to conclude that the bias for this OVB is towards zero?, as the correlations with the independent variables are evenly distributed, and we have reason to believe that there is a negative correlation with fatality rates.

Given our findings using our ANOVA testing and modeling, we have seen that our independent variables are in general ______.
If in general away from zero, we are likely artificially introducing significance in our model. towards is fine. 

TODO: Table of towards away conclusion / variables Spencer Song
