...
<!-- Added additional context on practical significance-->  
In the second model, unemployment percent and chronic respiratory disease death percent are shown to be statistically significant with a p value less than 0.01.  The variables of percent diagnosed with diabetes and the intercept are shown to be statistically significant but less so with a p value less than 0.1. With regards to practical significance, we see an increase in the coefficient for unemployment percent in the second model resulting in a stronger practical significance the variable would have on the fatality rate. Outside of the model, while unemployment does play a role in establishing a baseline for how the state's fatality rate is affected, during the pandemic we experienced unprecedented levels of unemployment(14%+) due to state wide shut downs and stay at home orders but did not see a significant rise in fatality rate as a result.  Therefore it would be safe to conclude that unemployment rate within each state does not have practical significance in this context. We do see mask ratio lose its statistical significance while still maintaining a similar level of practical significance to model one, however, logically speaking, the purpose of the mask policy is to prevent the user from catching or transmitting COVID-19 and since our model is looking to gauge the effect it has on fatality rates, we would not consider the mask ratio variable to be practically significant.  Continuing with practical significance, we see that chronic respiratory disease death percent has an abnormally large coefficient where we can conclude that for every percent change in the death rate for chronic respiratory disease we expect to see a 21.7% decrease in fatality rate.  In a national study done by the CDC, chronic respiratory diseases is ranked 4th in leading causes of death in the US.  On top of the large coefficient in the model, it is clear that chronic respiratory disease death percent has large practical significance as it poses a potential risk affecting up to 5% of the us adult population.  Lastly, we look at percent of population in each state diagnosed with diabetes. We can infer from the results that for every percent increase in the population that is diagnosed with diabetes, we see a 9.6% increase of fatality rate in the model.  In another study done by the CDC, they estimate a total of 13% of the US population has some form of diabetes, while the percent effect the variable has on fatality rate is smaller compared to chronic respiratory disease death rates, the sheer percentage of the population that this would apply to makes this variable practically significant.
...
## Examples of Omitted Variables Causing Bias
...
```{}
ovb_table <- data.frame("Food Quality" = 1:2, "Age" = c(21,15), "Name" = c("John","Dora"))
stargazer()
```
