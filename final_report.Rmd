...
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.cap="Changes in correlations between explanatory variables after removing some features in Model 2", fig.pos='!h', out.extra='trim={0 1cm 0 1cm},clip', fig.align="center"}
...
model_2_plot1 <- ggcorrplot(res_2_orig, type = "upper", method = "square", tl.cex = 8, hc.order = FALSE, colors = c("#6D9EC1", "white", "#E46726"), show.legend = FALSE) 

res_2_reduced <- cor(vars_2_reduced)
model_2_plot2 <- ggcorrplot(res_2_reduced, type = "upper", method = "square", tl.cex = 8, show.legend = FALSE, colors = c("#6D9EC1", "white", "#E46726"))
...
```{r, echo=FALSE, warning = FALSE, message = FALSE, fig.cap="Changes in correlations between explanatory variables after removing some features in Model 3", fig.pos='!h', out.extra='trim={0 1cm 0 1cm},clip', fig.align="center"}
...
model_3_plot1 <- ggcorrplot(res_3_orig, type = "upper", method = "square", tl.cex = 6, hc.order = FALSE, colors = c("#6D9EC1", "white", "#E46726"), show.legend = FALSE) 

res_3_reduced <- cor(vars_3_reduced)
model_3_plot2 <- ggcorrplot(res_3_reduced, type = "upper", method = "square", tl.cex = 6, show.legend = FALSE, colors = c("#6D9EC1", "white", "#E46726"))
...
TODO: don't display t-test results (remove coef test output)
TODO: decide which model to use and why

Independent variables: duration of mask policy, diabetes percentage, unemployment percentage.

Rewrite as $\text{Fatality Rate} = \beta_0 + \beta_1 \text{ attendance} + u$

Of all the variables we considered....

Chose one variable from each category

These are **so crucial** it would be ABSURD to omit them!! WE WOULD NEVER CHERRY PICK WHATTT sksksk

Diabetes: encompasses a bunch of other health conditions

No jobs = no health care = they die --> research shows low-income areas are more impacted by COVID
...
Independent variables: duration of mask policy, diabetes percentage, unemployment percentage, duration of stay-at-home policy, chronic respiratory disease death percentage, minority percentage.
```{r}
...
Given the models above, we have chosen to emphasize Model 2 because:
  - includes more explanatory features than Model 1
  - 3 has better numbers but we have concerns about causality, so we don't want to do that
  - "dummy overwhelming sksksk" - Spencer Song
...
df %>% 
  select(
    Fatality_Rate, mask_ratio, Diabetes_Diagnosed_Perc, Poverty_Perc, model_two_resids
  ) %>%
  GGally::ggpairs()
```
