...
Much of this data was collected  from a variety of sources, but we were able to join on FIPS (Federal Information Processing System) Codes. Our data was only primarily on a county level, so we aggregated our county data to a state level and converted the data to percentage rates to use for our regression. We made the choice to define "Fatality_Rate" as the total number of deaths / total number of cases from start to finish. We do NOT use COVID-19 deaths per capita, as are question of causality does not care for differences in the number of COVID-19 cases per state.
...
Policy: The covariates for both mask mandate and stay-at-home policy durations are of concern as they both show a positive linear relationship with fatality rates; however, they are small as both are less than 0.008.

Socioeconomic:

Miscellaneous: The covariates for senior percentages and ICU beds per 10000 actually appear to be of little concern. Neither seem to be overly problematic due to collinearity. 
...
1. **Limited Model**: Independent variables: duration of mask policy, percentage of seniors, poverty percentage.
```{r}
...
The limited model investigates the relationship between fatality rates and the duration of mask policies, the percentage of seniors, and the poverty percentage for each state. After running the coefficient test, we observe that both mask duration and poverty percentage are both statistically significant at a 95% significance level or greater. However, mask duration has a positive impact on predicted fatality rates, which goes against our assumption that states that implemented mask mandates longer would observe a decrease in predicted fatality rates. Except, the model calculates that a one day increase in mask mandates has a 0.005 increase in predicted fatality rates. For poverty percentage, the model describes that a one percentage increase in poverty leads to a 0.04 increase in predicted fatality rate. The VIF values for all three independent variables are all less than two, which indicates that there is minimal multicollinearity among them. With that said, the tolerance, or percent of variance in each variable that cannot be accounted for by the other predictors, of each variable is greater than 0.94. This suggests that there are variables not in the model that account for those predictors. The QQ plot shows that the model is under-representing the data as it has a light-tail indicating that the actual values are larger than the predicted values. Therefore, the next model will include additional variables to provide more clarity. 

1. **Model Two**: Independent variables: duration of mask policy, duration of home policy, percentage of seniors, poverty percentage, diabetes percentage, stroke death percentage, heart disease percentage, smoker percentage, chronic respiratory death percentage, minority percentage, unemployment percentage, crowding percentage.
```{r}
model_two <- lm(Fatality_Rate ~ mask_ratio + home_ratio + Diabetes_Diagnosed_Perc + Stroke_Death_Perc+ Heart_Disease_Death_Perc + Smoker_Perc + Chronic_Respiratory_Disease_Death_Perc + Senior_Perc + Poverty_Perc + log(Minority_Percentage) + Unemployment_Perc  + Crowding_Perc, data = df)
coeftest(model_two)
...
Model Two includes all the variables from the Limited Model and adds several more variables. The variables that are statistically significant at a 95% significance level or greater in this model include stroke death percentage (negative effect), chronic respiratory death percentage (negative effect), and unemployment percentage (positive effect). Concerning are the two first because as both increase, predicted fatality rates decrease, which goes against our assumptions that states with higher stroke or chronic respiratory death percentage would have higher fatality rates as those groups of people are more susceptible to COVID-19. The VIF values are only concerning for smoker percentage and poverty percentage as both are larger than five and could suggest that multicollinearity is present. The added variables do decrease the tolerance of each variable from the present model. This means that the variance of each variable is less accounted for by the variables not present in the model. The Anova test we ran to compare the fits of the Limited Model and Model Two calculates a p-value less than a 0.05 level of significance level. This shows that by adding the new nine variables leads to a significant improvement of predict fatality rates compared to the Limited Model.

1. **Model Three**: Independent variables: duration of mask policy, duration of home policy, percentage of seniors, poverty percentage, diabetes percentage, stroke death percentage, heart disease percentage, smoker percentage, chronic respiratory death percentage, asthma death percentage, obesity percentage, minority percentage, unemployment percentage, crowding percentage, multi-unit percentage, and ICU beds per 10,000 people.
