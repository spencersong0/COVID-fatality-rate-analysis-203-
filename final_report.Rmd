...
In the Limited Model, Table 1 shows that the Mask Ratio variable is statistically significant with a p value less than 0.05. The model also returns unemployment percent as statistically significant. With respect to practical significance, for every percent of the time the mask policy was enacted, all else held constant, we expect to see a 0.3% increase in fatality rates.  Which is counterintuitive considering we were expecting the mask policy to contribute to lowering COVID-19 deaths. Another variable that was tested to be statistically significant is the unemployment percentage. The coefficient of 0.195 implies that without any effects from other variables, a one percent increase in unemployment effectively increases the fatality rate by 19.5% thus pointing to a large practical significance.  
...
“Food Quality” will likely have a high negative correlation with diabetes diagnoses and a negative correlation with unemployment percentages. Because the independent variables have a positive impact on fatality rates, have a negative correlation with our omitted variable, and "Food Quality" likely has a negative impact on fatality rates, we conclude that the direction of the OVB is away from zero, as we have likely overstated the effects of these independent variables on fatality rates.
“Food Quality” will likely have a negative correlation with minority percentages, and as minority has a negative causal impact on fatality rates and "Food Quality" likely has a negative correlation with fatality rates, the direction of the OVB will be towards zero, and we have likely understated the impact of minority percentage.
We have summarized our conclusions for the directions of the omitted variables biases in Table 3. For the remaining omitted variables, reference Table 3 for a summary of the directions.
...
2. \textbf{Perception of COVID-19 Severity:} A survey value of the average perception of how severe or fatal COVID-19 is.
...
“Perception of COVID-19 Severity” will likely have no correlation with diabetes diagnoses, unemployment percentages, and minority percentages. “Perception of COVID-19 Severity” will likely have a strong positive correlation with state mask policies, a positive correlation with state stay at home policies, and a positive correlation with chronic respiratory disease percentages. Table 3 summarizes the direction of the omitted variable bias.
We don’t necessarily have a great replacement for this variable. We would have assumed that policy variables would be a replacement, but we have inverse results in both of our policies, so it isn’t clear how we could find an accurate replacement for this variable.

3. \textbf{\% registered Republican:} The percentage of registered voters in a state who are registered as Republican.

> We believe that “% registered Republican” will have a positive correlation with fatality rates. We believe that higher Republican percentages will be correlated with less COVID-19 resources and policies in general from state government.
“% registered Republican” will likely have no correlation with diabetes percentages and chronic respiratory percentages. This variable will also have a strong negative correlation with state mask policies and stay-at-home policies.
This variable will likely have a strong negative correlation with minority percentages and unemployment rates. Table 3 summarizes the direction of the omitted variable bias.
This is also a difficult line of reasoning, as we have seen from our regression that policies have had mixed impacts on fatality rates, which suggests our reasoning may be incorrect. So the bias for this variable could very easily be different than what we assume. Our policy variables are a good replacement because higher percentages of Republicans likely correlates with the state policies implemented.

4. \textbf{Population \% in Urban Zones:} The percentage of the state’s population that live in urban zones.

> We believe that this variable will have a positive correlation with fatality rates, as clusters of individuals would likely be more quickly and severely impacted by COVID-19.
This variable likely has no correlation with unemployment percentages and diabetes percentages.
This variable likely has a positive correlation with minority percentages, as urban zones correlate with higher minority rates.
This variable likely has a positive correlation with chronic respiratory diseases---if more people live in urban zones, there are more people exposed to local air pollution, so there are also likely more individuals suffering from chronic respiratory diseases. 
This variable also likely has a positive correlation with mask policy and stay-at-home polices. Higher populations in urban zones will likely correlate with more necessary regulations across a state for COVID-19. 
Table 3 summarizes the direction of the omitted variable bias. We do not have a variable in our model that can represent this statistic, as crowding is focused more on the specifics of household-level population distributions.
...
“Exercise Rates” will likely have no correlation with state mask policies, stay-at-home policies, minority percentages, and unemployment percentages. This omitted variable will likely have a negative correlation with diabetes and chronic respiratory diseases.
Table 3 summarizes the direction of the omitted variable bias.
We don’t necessarily have a variable that can replace activity in a population, but our health variables are likely indicators that would impact fatality rates.

\begin{table}[!htbp] \centering 
  \caption{OVB Analysis Results (Away From Zero vs Towards Zero)} 
  \label{} 
\begin{tabular}{@{\extracolsep{5pt}} ccccccc} 
\\[-1.8ex]\hline 
\hline \\[-1.8ex] 
 Independent Var. & Food & Perception of
          & \%\ Registered & Population \% & Exercise\\ Variable Names & Quality & COVID-19 Severity & Republican & in Urban Zones & Rates  \\ 
\hline \\[-1.8ex] 
Mask Policy & None & Towards & Towards & Away & None \\ 
Diabetes \% & Away & None & None & None & Away \\ 
Unemployment & Away & None & Towards & None & None \\ 
Stay-at-Home Policy & None & Away & Away & Towards & None \\ 
Minority \% & Towards & None & Away & Towards & None \\
Chronic Resp. Disease  \% & None & Away & None & Towards & Towards \\
\hline \\[-1.8ex] 
\end{tabular} 
\end{table}

Given our findings using our Anova testing and modeling, we are likely artificially introducing some significance in our model through omitted variable bias. Considering our model and significance results, the primary covariate of concern is Percent of Adults Diagnosed with Diabetes. Percent diabetes was found to be significant but at the lowest significance level with a p-value of 0.094. Because all the non-zero OVBs are away from zero, a correct model would most likely not find that diabetes is statistically or practically significant.
