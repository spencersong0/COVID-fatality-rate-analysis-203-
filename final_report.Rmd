...
\textbf{Socioeconomic}: TODO <!--@KEVIN CAN YOU ADD SOME STUFF HERE TY--> move senior percentages to this section..
...
\textbf{Socioeconomic}: TODO: Describe logarithmic transformation applied to the data. Reference the scatterplot figure!! senior percentage should not take any transformations
...
plot4 <- ggplot(df, aes(y = Fatality_Rate, x = log(Minority_Percentage))) +
 geom_point() + stat_smooth() + labs(x = '% Minority', y = 'log(Fatality Rate)', title = 'Fatality Rate vs % Minority with Transformation')
plot3 / plot4
```
...
TODO: Introduction to what tests we ran and why? maybe mention that we used the robust standard errors?
